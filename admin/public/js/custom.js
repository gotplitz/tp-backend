...
(function ($) {
    ...
    /*----------------------------------------------------*/
        ...
        var uploadButtonb = {
            $buttonb: $('.uploadButtonb-input'),
            $nameFieldb: $('.uploadButtonb-file-name'),
        };

        uploadButtonb.$buttonb.on('change', function () {
            _populateFileFieldb($(this));
        });

        function _populateFileFieldb($buttonb) {
            var selectedFile = [];
            for (var i = 0; i < $buttonb.get(0).files.length; ++i) {
                selectedFile.push($buttonb.get(0).files[i].name + '<br>');
            }
            uploadButtonb.$nameFieldb.html(selectedFile);
        }
        /*----------------------------------------------------*/
        /*  Gallery
    /*----------------------------------------------------*/
        ...
        var uploadButtons = {
            $buttons: $('.uploadButtons-input'),
            $nameFields: $('.uploadButtons-file-name'),
        };

        uploadButtons.$buttons.on('change', function () {
            _populateFileFields($(this));
        });

        function _populateFileFields($buttons) {
            var selectedFiles = [];
            for (var i = 0; i < $buttons.get(0).files.length; ++i) {
                selectedFiles.push($buttons.get(0).files[i].name + '<br>');
            }
            uploadButtons.$nameFields.html(selectedFiles);
        }
